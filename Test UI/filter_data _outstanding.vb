...
Imports Test_UI.monitoring
Public Class filter_data__outstanding
    Dim data As New Db
    ...
    Private Sub filter_data__outstanding_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.ControlBox = False
    ...
    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        ...
        If item_cd.Checked Then
            ...
            For i = 0 To list_item_cd.Count - 1
                ...
                adpt.Fill(ds, "emp")
                adpt.Fill(tbb)
                Dim code As String, code_1 As String, desk As String
                Dim prodplan As Integer, stock As Integer, pcs_day As Integer
                ...
                If tbb.Rows.Count > 0 Then
                    ...
                    desk = tbb(0)(3)
                Else
                    MsgBox("Item dan Kategori tidak sesuai!")
                    Return
                End If
                ...
                Dim adpt2 As New MySqlDataAdapter("select * from outstanding where item_code = '" & code & "'", con)
                Dim ds2 As New DataSet
                Dim tbb2 As New DataTable
                adpt2.Fill(ds2, "emp2")
                adpt2.Fill(tbb2)
                If tbb2.Rows.Count() > 0 Then
                    For j = 0 To tbb2.Rows.Count - 1
                        If tbb2.Rows.Count > 1 Then
                            If j > 0 Then
                                baris += 1
                                osheet.Range("I" & baris.ToString).Value = tbb2(j)(2)
                                osheet.Range("J" & baris.ToString).Value = tbb2(j)(3)
                                osheet.Range("K" & baris.ToString).Value = tbb2(j)(4)
                                osheet.Range("L" & baris.ToString).Value = Format(Date.Parse(tbb2(j)(5)), "dd-MMM-y")
                            Else
                                baris += 1
                                baris_baru = baris
                                osheet.Range("B" & baris.ToString).Value = no
                                ...
                                osheet.Range("G" & baris.ToString).Value = stock
                                osheet.Range("H" & baris.ToString).Value = in_days
                                osheet.Range("I" & baris.ToString).Value = tbb2(j)(2)
                                osheet.Range("J" & baris.ToString).Value = tbb2(j)(3)
                                osheet.Range("K" & baris.ToString).Value = tbb2(j)(4)
                                osheet.Range("L" & baris.ToString).Value = Format(Date.Parse(tbb2(j)(5)), "dd-MMM-y")
                                osheet.Range("M" & baris.ToString).Value = Format(Date.Now().AddDays(CInt(in_days)), "dd-MMM-y")
                                osheet.Range("E" & baris.ToString).Value = prodplan
                                no += 1
                            End If
                        Else
                ...
                osheet.Range("B3" & ":L" & (baris.ToString)).Borders.LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous

                obook.Windows.Application.ActiveWindow.Zoom = 85
                osheet.Range("B3" & ":L" & (baris.ToString)).EntireColumn.AutoFit()
            Next
